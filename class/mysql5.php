 ...
 * MySQL数据库调用类，仅支持PHP5
 * 
 * @version 	2010年2月25日10:43:17
 * @update 2010-2-25 22:02:47 修改了Query函数，改为 mysql_fetch_assoc 
 * @author 	Zerolone
 */
...
class Database {
	...
	public function Query() {
		...
		if(function_exists('mysql_unbuffered_query')){
			$temp_query	= mysql_unbuffered_query( $this->SqlStr, $this->ResID );
		}else{		
			$temp_query = mysql_query ( $this->SqlStr, $this->ResID );
		}
		/**/
		...
		//返回结果数组
		if ($temp_query){			
//			$this->ResultArr[]=mysql_fetch_assoc ( $temp_query );		
			while($queryArr = mysql_fetch_assoc($temp_query)) {		
				$this->ResultArr[]= $queryArr ;
				$returnValue=TRUE;
			}
		}		
		return $returnValue;
