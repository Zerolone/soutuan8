function mywin()
{
	...
	this.maxWins		= 10;				//最大窗口数
	this.tagTitleWidth	= 150;				//标签宽度
	this.indentWidth	= 8;				//标签缩进宽度
	...
	function activewin(oEl)                 //激活窗口
	{
		...
		this.currentwin.wintitle.style.backgroundImage = 'url(images/page_unselect.gif)';

		//Zerolone Edit By 20040908 In GZ
		//功能：失去焦点后恢复
		this.currentwin.wintitle.style.fontWeight='';	

		oEl.wintitle.style.zIndex = tempzindex;
		oEl.style.display = "";
		oEl.wintitle.style.backgroundImage = 'url(images/page_select.gif)';

		//Zerolone Edit By 20040908 In GZ
		//功能：点击加粗
		oEl.wintitle.style.fontWeight='bold';	
	...
	function addwin(url,title)                                        //方法的具体实现
	{
		...
		oDIV.height = "100%";
		oDIV.cellSpacing=0;
		oDIV.insertRow().insertCell().innerHTML = "<iframe src='"+url+"' width='100%' height='100%' frameborder='0' marginwidth='0' marginheight='0'></iframe>";

		var oTitle = window.document.createElement( "SPAN" );
		oTitle.className ='wintitle';
		oTitle.style.width = this.tagTitleWidth;
		oTitle.style.backgroundImage = 'url(images/page_select.gif)';
